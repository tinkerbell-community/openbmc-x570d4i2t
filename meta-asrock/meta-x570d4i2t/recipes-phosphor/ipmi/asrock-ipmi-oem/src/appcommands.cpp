...
//   - GetDeviceId (0x01) returns the correct ASRock manufacturer ID / product
//   ID
//     from dev_id.json plus the actual running firmware version string from
//     D-Bus.
//   - GetSystemGuid (0x34) returns the board UUID sourced from the D-Bus
//     inventory UUID property rather than a synthesised value.
//
// Method references:
//   bmc-analyze §NETFN_APP table (g_App_CmdHndlr, confirmed codes
//   0x01/0x08/0x34) bmc-analyze §Core IPMI Daemon: IPMI.conf
//   (manuf_id=0x00C1D6, prod_id=0x1003) bmc-analyze §Firmware Identity: device
//   ID 0x20 / revision 0x81
...
static std::string getActiveBmcVersion() {
  try {
    ...
    for (const auto &path : paths) {
      try {
        std::string svc = ipmi::getService(*dbus, softwareVerIntf, path);

        ipmi::Value actV = ipmi::getDbusProperty(*dbus, svc, path,
                                                 softwareActIntf, "Activation");
        const auto &actStr = std::get<std::string>(actV);
        if (actStr.find("Active") == std::string::npos) {
          continue;
        }
        ipmi::Value verV =
            ipmi::getDbusProperty(*dbus, svc, path, softwareVerIntf, "Version");
...
ipmiGetDeviceId(ipmi::Context::ptr & /*ctx*/) {
  phosphor::logging::log<phosphor::logging::level::INFO>("Running GetDeviceId");
  ...
  try {
    ...
    if (ifs.is_open()) {
      ...
      deviceRevision = static_cast<uint8_t>(j.value("revision", 1) & 0x0F) |
                       (deviceRevision & 0xF0);
      additionalSupport = j.value("addn_dev_support", additionalSupport);
      uint32_t mfgId = j.value(
          "manuf_id",
          static_cast<uint32_t>((mfgByte2 << 16) | (mfgByte1 << 8) | mfgByte0));
      ...
      uint32_t prodId = j.value(
          "prod_id", static_cast<uint32_t>((prodByte1 << 8) | prodByte0));
  ...
  phosphor::logging::log<phosphor::logging::level::DEBUG>(
      "GetDeviceId", phosphor::logging::entry("FW_MAJOR=%u", fwMajor),
  ...
  return ipmi::responseSuccess(deviceId, deviceRevision, fwMajor, fwMinorBcd,
                               ...
                               mfgByte1, mfgByte2, prodByte0, prodByte1, auxFw);
}
...
ipmiGetSelfTestResults(ipmi::Context::ptr & /*ctx*/) {
  ...
  try {
    auto dbus = getSdBus();
    ipmi::getService(*dbus, "xyz.openbmc_project.Logging.IPMI",
  ...
  try {
    auto dbus = getSdBus();
    ipmi::getService(*dbus, "xyz.openbmc_project.EntityManager",
  ...
  phosphor::logging::log<phosphor::logging::level::DEBUG>(
      "GetSelfTestResults", phosphor::logging::entry("RESULT=0x%02X", result),
...
ipmiGetSystemGuid(ipmi::Context::ptr & /*ctx*/) {
  ...
  try {
    ...
    ipmi::Value v =
        ipmi::getDbusProperty(*dbus, svc, chassisSystemObj, uuidIntf, "UUID");
  ...
  phosphor::logging::log<phosphor::logging::level::DEBUG>(
      "GetSystemGuid", phosphor::logging::entry("UUID=%s", uuidStr.c_str()));
